...
서버(`make run-server`)와 클라이언트(`make run-client`)가 실행된 상태에서, 실제 플레이 흐름은 다음과 같습니다.
...
```text
터미널 B> 닉네임을 입력하세요: Alice  
터미널 C> 닉네임을 입력하세요: Bob
터미널 D> 닉네임을 입력하세요: Jack
```
...
```text
#1
당신은 폐허가 된 도시 한복판, 쓰러진 버스 안에서 눈을 뜹니다. 무엇을 하시겠습니까?
1. 창문을 깨고 밖으로 나간다.
2. 버스 안을 수색한다.
3. 창문 틈으로 주변을 살핀다.
...
```
```text
서버: Alice(2), Bob(1), Jack(1) → 1번으로 분기  
[결과] HP -0, EXP +10, Gold +5, Attack +1, Defense -1
다음 스테이지로 이동합니다...  
```
...
```text
세상을 한순간에 바꿀 수 있는 힘이 손에 있었지만, 당신은 그것을 내려놓았습니다.  
